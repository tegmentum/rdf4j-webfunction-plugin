...
public class TestWfTreeE2E {
    ...
    /**
     * Currently ignored — the WIT host-callback marshalling in {@link HostCallbacks}
     * returns raw Java {@code Object[]} whereas the wasmtime4j component linker
     * expects properly-typed {@code WitValue}/{@code ComponentVal} instances
     * (WitResult, WitRecord, WitList, WitString, etc.). Attempting instantiation
     * of wf_tree.wasm right now surfaces:
     *
     *   "instance export `execute-query` has the wrong type: function
     *    implementation is missing"
     *
     * The wire-up (linker binding at the right witPath, strategy ThreadLocal
     * for the callback context, config knobs, sub-query execution via
     * strategy.precompile) is all in place. What remains is rewriting the two
     * Object[] returns in HostCallbacks to build WIT-typed values with the
     * same discipline WitValueMarshaller applies to VALUE_TYPE. A dedicated
     * session iterating against this test unblocks it.
     */
    @Ignore("host-callback WIT marshalling in HostCallbacks needs WitValue wrappers — see comment above")
    @Test
