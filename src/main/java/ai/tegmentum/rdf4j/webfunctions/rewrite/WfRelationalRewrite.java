...
import ai.tegmentum.rdf4j.webfunctions.rewrite.FederationRegistry.Column;
import ai.tegmentum.rdf4j.webfunctions.rewrite.FederationRegistry.FederationSource;
import ai.tegmentum.rdf4j.webfunctions.rewrite.FederationRegistry.RelationalConfig;
 ...
 * wf_fetch URL &rarr; short-circuit; unknown name &rarr; leave the
 * SERVICE alone.
 *
 * <h2>v0.3 unification</h2>
...
public final class WfRelationalRewrite implements QueryOptimizer {
    ...
    public void optimize(final TupleExpr tupleExpr, final Dataset dataset, final BindingSet bindings) {
        rewrites = 0;
        if (federationRegistry == null || federationRegistry.isEmpty()) return;
    ...
    private final class Walker extends AbstractQueryModelVisitor<RuntimeException> {
        ...
        public void meet(final Service service) {
            ...
            if (entry.sourceType() != FederationRegistry.SourceType.WF_RELATIONAL) {
                return;
            }
            final Optional<RelationalConfig> cfgOpt = entry.relationalConfig();
            if (cfgOpt.isEmpty()) return;
            final RelationalConfig cfg = cfgOpt.get();
            ...
            final TupleExpr replacement = buildService(subjectVar, entry, cfg, columns);
    ...
    private TupleExpr buildService(final Var subjectVar,
        ...
        body.add(sp(oVar.clone(),
                constVar(VF.createIRI(WF_NS + cfg.subjectColumn())),
                subjectVar.clone()));
     ...
     */
    static String buildDescriptorJson(final FederationSource entry, final RelationalConfig cfg) {
        ...
        if (cfg.anchor() != null && cfg.anchor().anchorClass() != null) {
            anchor.put("class", cfg.anchor().anchorClass());
        }
        ...
        if (cfg.iriTemplate() != null) root.put("iri_template", cfg.iriTemplate());
        if (cfg.schemaVersion() != null) root.put("schema_version", cfg.schemaVersion());
